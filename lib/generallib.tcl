# perflib.tcl - library functions for performance analysis.
...
proc gnuplot_file {m_filename png_filename plot_filename} {
	...
	if {!$error} {
		set res ""
		set res_stderr ""
		# catch {set res [exec $env(FOURNT_HOME)/4nt.exe /c plotfile.bat $m_filename $png_filename $plot_filename]} res_stderr

		set reslogs_dir [file dirname $png_filename]
		set old_dir [pwd]
		cd $reslogs_dir

		# 5-9-2006 (vreen00) padnaam kan te lang worden voor gnuplot, dus copy naar temp-dir
...
# 24-10-2010 added from FB log2analysis.tcl
# @todo format eigenlijk vervangen door id-functie.
proc get_array_values {ar_name args} {
  upvar $ar_name ar
  # struct::list mapfor el $args {string trim $ar($el)}
...
#######################################################
# General helper procs, moved from playingscheme.ndv  #
#######################################################
