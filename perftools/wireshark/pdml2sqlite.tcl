...
proc main {argv} {
  set tmp_db "/tmp/kg.db"
  # set src_dir "/media/nas/aaa/kg"
  make_db $tmp_db
  # read_data [file join $src_dir 1109a-displayed.pdml]
  # set filename 1109a.pdml
  set filename [lindex $argv 0]
  read_data [file join $filename]
  
  set_first_last_packet
  det_roundtrips
  
  db close
  # file delete [file join $src_dir kg.db]
  file delete [det_db_name $filename]
  # file rename $tmp_db [file join $src_dir kg.db]
  file rename $tmp_db [det_db_name $filename]
}
...
proc make_db {tmp_db} {
  ...
  db eval "create table roundtrip (req_num integer, resp_num integer, stream integer, duration float)"
...
proc handle_packet {packet} {
  ...
  if {$tcpstream == ""} {
    set tcpstream [det_tcpstream $ipsrc $portsrc $ipdst $portdst] 
  }
  # breakpoint
...
proc det_roundtrips {} {
  ...
  db eval "update packet set reqresp = 'REQ'
           ...
           and (details like 'GET%' or details like 'POST%' or details like '\[truncated\] POST%' or details like '\[truncated\] GET%')"
  ...
  db eval "insert into roundtrip
           select p1.packetnum, p2.packetnum, p1.tcpstream, p2.timestamp-p1.timestamp
...
# 15-2-2012 NdV in wireshark 1.0.0 geen tcp.stream, wel goede herkenning van bepaalde HTTP-POST pakketten, die latere versies (vanaf 1.1.0 of eerder) niet meer ondersteunen.
# test of gegenereerde tcpstream gelijk is aan die van de nieuwere versies.
# global var ar_stream(ip1,port1,ip2,port2): fill both sides when new one added.
# global next_stream_id
set next_stream_id 0
proc det_tcpstream {ipsrc portsrc ipdst portdst} {
  global ar_stream next_stream_id
  if {$portsrc == ""} {
    return "" 
  }
  if {[array get ar_stream "$ipsrc,$portsrc,$ipdst,$portdst"] == {}} {
    set ar_stream($ipsrc,$portsrc,$ipdst,$portdst) $next_stream_id
    set ar_stream($ipdst,$portdst,$ipsrc,$portsrc) $next_stream_id
    incr next_stream_id
  }
  return $ar_stream($ipsrc,$portsrc,$ipdst,$portdst)  
}
