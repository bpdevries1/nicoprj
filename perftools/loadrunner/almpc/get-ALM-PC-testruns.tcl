...
# this is done via the new REST API on port 80 (get-ALM-PC-tests.tcl uses port 8080, old ALM/QC)
...
proc main {argv} {
  ...
  set options {
    {dir.arg "c:/PCC/Nico/ALMdata" "Main directory to put downloaded ALM files and SQLite DB"}
  ...
  #dict set config nodownload [:nodownload $dargv]
  #dict set config just1 [:just1 $dargv]
...
proc download_runs {prj_rootdir} {
  ...
  # alm_url = http://wsrv3889.rabobank.corp/loadtest/rest
  # http://wsrv3889.rabobank.corp/loadtest/rest/domains/RI/projects/Ri_Shared_environment/Runs
...
proc alm_login {} {
  ...
  file delete cookies.txt
  set CURL_BIN [:curl $config]
  # deze zou goed moeten gaan. Zo niet, dan error en crash.
  #exec -ignorestderr $CURL_BIN -c cookies.txt --header "Authorization: Basic [:auth_base64 $config]" "[:url $config]/authentication-point/authenticate"
...
proc handle_run_details {db prj_rootdir runid testrun_id} {
  ...
  # http://wsrv3889.rabobank.corp/loadtest/rest/domains/RI/projects/Ri_Shared_environment/Runs/381/results
  set run_dir [file join $prj_rootdir $runid]
  file mkdir $run_dir
