  ...
  foreach row [db_query $conn $query] {
    ...
    if {$staging_ip != $prev_staging_ip} {
      if {$prev_staging_ip != "<none>"} {
        ...
        make_hosts_lines $f $prev_staging_ip $domains
      } else {
        # none, first item 
      }
...
# put max 9 items on each hosts line, windows does not allow more, and will silently ignore the rest.
proc make_hosts_lines {f staging_ip domains} {
  foreach lst [partition_all 9 $domains] {
    puts $f [format "%-15s %s" $staging_ip [join $lst " "]]
  }
}
