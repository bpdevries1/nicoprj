proc main {} {
  ...
  puts $fo [join [list logfilename ts_cet test filename runtime iops mbps avg_latency_msec] "\t"]
  foreach logfilename [glob -directory $rootdir Results_*.txt] {
...
#	using 8KB random IOs    
    ...
    if {[regexp {histogram:} $line]} {
      puts $fo [join [list $logfilename $ts_cet "$readwrite-$nkb-$accesstype" $filename $seconds $iops $mbps $avg_lat_msec] "\t"]
      foreach var $lst_vars {
        set $var "<none>"
      }
    }
