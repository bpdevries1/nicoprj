...
itcl::class WorkflowStatusLogReader {
  ...
  public proc new {db} {
 		set instance [uplevel {namespace which [WorkflowStatusLogReader #auto]}]
    $instance init $db
    return $instance  
  }
  ...
  public method can_read {filename} {
    if {[regexp {WorkflowStatus} $filename]} {
        return 10 
    }
    return 0
  }
...
#2010-09-02,12:31:00.027,2010-09-02,12:31:01.323,IAS.IASJob,1.296,0.000
    ...
    while {![eof $fi]} {
       ...
       set lst [split $line ","]
       if {[llength $lst] == 6} {
         lassign $lst date1 time1 date2 time2 url elapsed
       } else {
         continue  
       }
