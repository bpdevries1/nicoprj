...
proc graph_avail {r} {
  # total times divided by 11
  $r query "select country, date, avail
            from runstat"
  $r qplot {title "Average daily availability per country - 1"
            x date y avail
            ylab "Availability"
            ymin 0
            geom line-point
            colour country
            width 11 height 7}
  $r qplot {title "Average daily availability per country - 2"
            x date y avail
            ylab "Pageload (sec)"
            ymin 0
            geom line
  ...
  $r query "select date, avg(avail) avail
            from runstat
            group by 1"
  $r qplot {title "Daily average availability"
            x date y avail
            ylab "Availability"
            ymin 0
            geom line
  ...
  # page details
  $r query "select date, country, page_seq, avail
            from pagestat"
  $r qplot {title "Daily page availability per page and country"
            x date y avail
            ylab "Availability"
            ymin 0
            geom line
  ...
  $r query "select date, page_seq, avg(avail) avail
            from pagestat
            group by 1,2"
  $r qplot {title "Daily average availability per page - 1"
            x date y avail
            ylab "Availability"
            ymin 0
            geom line
            facet page_seq
            width 11 height 12}
  $r qplot {title "Daily average availability per page - 2"
            x date y avail
            ylab "Availability"
            ymin 0
            geom line-point
  ...
  foreach country {NL DE FR UK US} {
    ...
    $r qplot [list title "Daily availability per page - $country" \
              x date y avail \
              ylab "Availability" \
              ymin 0 \
              geom line \
