...
public class IfController extends GenericController implements Serializable {
	...
	private ThreadLocal scopes = new ThreadLocal();
	...
	private boolean evaluateCondition(String cond) {
		logger.debug("    haalConditie2() : [" + cond + "]");

		String resultStr = "";
		boolean result = false;

		// first check if the string is already 'true' or 'false', then javascript won't be necessary.
		// javascript has significant overhead.
		...
		// Context must be associated with current thread.
		// Context cx = Context.enter();

		// use threadlocals
		Context cx = (Context) contexts.get();
		Scriptable scope = (Scriptable) scopes.get();
		if (cx == null) {
			logger.info("scope is null => initialise");
			cx = Context.enter();
			contexts.set(cx);
			// scope is also still null
			scope = cx.initStandardObjects(null);
			scopes.set(scope);
		}
		...
		try {
			...
			// logger.debug("    >> evalueer conditie -  [ " + cond + "] results is  [" + result + "]");
			logger.info("    >> evalueer conditie -  [ " + cond + "] results is  [" + result + "]");
		} catch (Exception e) {
