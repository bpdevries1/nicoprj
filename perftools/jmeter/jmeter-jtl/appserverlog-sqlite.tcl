...
proc main {argv} {
  ...
  puts "dbfile: $dbfile"
  create_db $dbfile
  handle_appserverlog $acclogfile
  finish_db
}
...
proc create_db {dbfile} {
  ...
  set fields [list filename ts name value]
  db eval "create table logline ([join $fields ", "])"
  
  set fields [list filename ts pid name value]
  db eval "create table processline ([join $fields ", "])"
...
proc finish_db {} {
  ...
  db eval "create index ix_logline_ts on logline (ts)"
  db eval "create index ix_process_ts on processline (ts)"
  puts "Creating indexes finished, closing db"
  db close
}

proc handle_appserverlog {acclogfile} {
