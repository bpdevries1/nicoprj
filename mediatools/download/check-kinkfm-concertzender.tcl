...
proc main {argv} {
  ...
  while {$sec_date < $now} {
    check_x Rated $outdir_root $sec_date
    check_x Ray $outdir_root $sec_date
...
# check if file to download exists and has correct size.
proc check_x {sub outdir_root sec_date {hour none}} {
  ...
  if {[file exists $to_path]} {
    if {[filesize_ok $to_path]} {
      ...
      if {[file size $to_path] == 64} {
        ...
        puts "Already downloaded, but size not ok: $to_path: [size_mb $to_path] mb"
        # file rename $to_path "$to_path.sizenotok"
        set to_download 1
      }
    }
  } else {
...
proc size_mb {path} {
  format "%.3f" [expr 1.0 * [file size $path] / (1024*1024)]
  
}

proc filesize_ok {path} {
  set ranges {{137 138} {68 69}}
  set size_mb [expr [file size $path] / (1024*1024)]
  set result 0
