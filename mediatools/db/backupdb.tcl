...
proc backup_db {dargv} {
  ...
  set dbname [:db $dargv]
  set backup_root "/media/nico/Iomega HDD/backups/databases/$dbname"
  file mkdir $backup_root
  set filename "$backup_root/${dbname}-[clock format [clock seconds] -format "%Y-%m-%d--%H-%M-%S"].sql"
  # set res [exec mysqldump -h localhost -u [:user $dargv] -p[:pw $dargv] scheids | gzip -9 > $filename]

  set res [exec pg_dump -d $dbname -U [:user $dargv] -h localhost -f $filename --blobs --clean --create]
  log info "Result: $res"
